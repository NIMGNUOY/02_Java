...
public class PrintExample {
	...
	public static void main(String[] args) {
		...
		System.out.println(); // 내용 없는 println() : 단순 줄 바꿈 용도
		...
		System.out.printf("%d\n", iNum3); // 3
		System.out.printf("%5d\n", iNum3); // 5칸 공간을 확보 후 오른쪽 정렬
		System.out.printf("%-5d\n", iNum3); // 5칸 공간을 확보 후 왼쪽 정렬
		
		// 소수점 자리 제어 (반올림 처리)
		System.out.printf("%f\n", 10/4.0); // 2.500000 (%f -> 소수점 아래 6자리까지 출력)
		System.out.printf("%.2f\n", 10/4.0); // 2.50 (%f -> 소수점 아래 2자리까지 출력)
		System.out.printf("%.0f\n", 10/4.0); // 3 소수점 아래를 표현 X (올림으로 3출력)
		
		
		// 문자, 문자열, boolean
		boolean isTrue = false;
		char ch = '조';
		String str = "안녕하세요"; // 참조형 (기본 자료형 X)
								   // 맨앞에 대문자, "" (쌍따옴표) 사용해야 함
		...
		// '\' 출력 : \\ (백슬래시 출력방법)
		System.out.println("\\");  //  '\' 하나 출력
		
		//  \o/
		System.out.println("\\o/");
		
		System.out.println("a\tb\tc\td");  // \t : tab 출력(정해진 공간만큼 띄어쓰기)
										   // 한 공간씩 표현X , 한번에 공간차지(드래그로 확인)
