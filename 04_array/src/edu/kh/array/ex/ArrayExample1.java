...
public class ArrayExample1 {
	...
	public void ex1() {
		...
		// ex) int[] arr2 = new double[5];  --> 자료형이 다르기때문에 Error
		
		// heap 영역에 생성된 int[]의 시작주소를 
		// stack 영역에 생성된 arr 변수에 대입
		
		// -> arr 변수가 int[] 배열을 참조하게 됨
		//	 그래서 arr 을 참조형이라고 함.
		...
		// arr[0], arr[1], arr[2] 은 int 자료형이기 때문에 정수값 대입 가능
		arr[0] = 10;
		arr[1] = 50;
		arr[2] = 1000;
	...
	public void ex2() {
		...
		// 배열의 길이 (몇 칸 인가) : 배열명.length -> JS 와 같음
	...
	public void ex3() {
		...
		// height 에 heap 영역에 새로 생성된 double 자료형 5칸짜리
		// double[] 의 시작 주소를 대입
	...
	public void ex4() {
		...
		int sum = 0;
		int max = score[0]; // 최고점을 저장할 변수
		int min = score[0]; // 최저점을 저장할 변수
	...
	public void ex5() {
		// 배열 선언과 동시에 초기화 {} : 중괄호 사용
		// cf ) JS 는 배열 초기화 시 [] : 대괄호 사용
		...
		int[] arr2 = new int[4]; // 0 0 0 0 --> 기본값
		System.out.println(Arrays.toString(arr2));
		// int 형 배열 4칸짜리 생성 후 각 index에 값을 대입하지
		// 않았으므로 JVM 에 의해서 int 형 기본값인 0으로 각각 초기화
	...
	public void ex8() {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			if (arr[i].equals(input)) {  // String 비교시 equal() 사용
				System.out.println(i + "번째 index에 존재");
				flag = true;
			}
	...
	public void ex9() {
		...
		// 문자하나를 입력받아 일치하는 문자가 char 배열에 몇개 존재하는지 확인
