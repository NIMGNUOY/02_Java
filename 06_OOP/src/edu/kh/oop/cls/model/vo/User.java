...
public class User {
	...
	// 생성자 : new 연산자를 통해서 객체가 생성될 때 생성된
	...
	// 필드를 전부 초기화하는 목적의 매개 변수 생성자
	public User ( String userId, String userPw, String userName,
					int userAge, char userGender) {
		
		// this() 생성자
		// 같은 클래스의 다른 생성자를 호출할 때 사용
		// 생성자 내에서 반드시 첫번재 줄에 작성해야만 한다! 위에 다른 코드 쓰면 에러
		// Constructor call must be the first statement in a constructor
		// 왜 사용하는가?
		// 중복코드 제거 , 코드길이 감소 , 재사용성 증가 ...
		// 가독성이 좀 떨어지는 경우가 있다(단점)
		
		this(userId, userPw); // -> 101번 째 줄의 생성자
		//this.userId = userId;
		//this.userPw = userPw;
		this.userName = userName;
		this.userAge = userAge;
	...
	// *** 오버로딩(Overloading) ***
	// - 클래스내에 동일한 이름의 메서드(생성자도 포함)를
	//  여러개 작성하는 기법
	// -> 하나의 이름으로 여러 기능을 수행할 수 있게 하는 것
	
	// [오버로딩의 조건]
	// 1) 메서드의 이름이 동일
	// 2) 매개변수의 개수, 타입, 순서 중에 1개라도 달라야 함.
	...
	public User (int userAge) {}  // -> 매개변수의 개수는 같지만
	//									타입이 다름 즉, 오버로딩 성립
	...
	// -> 매개변수의 개수, 타입이 모두 같아서 오버로딩 불가!
	// -> 변수명은 신경쓰지 않는다. 타입, 순서만 본다
}
