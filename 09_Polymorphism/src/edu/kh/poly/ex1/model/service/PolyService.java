...
public class PolyService {
	...
	public void ex1() {
		...
		// 객체 배열 : 같은 객체 참조 자료형의 변수를 하나의 묶음으로 다루는 것
					// 부모 타빙 참조 자료형의 변수를 하나의 묶음으로 다루는 것
	...
	// 전달받은 Car 또는 자식객체 (Tesla, Spark)의 엔진, 연료, 바퀴갯수 
	// 출력하는 메서드 
	// 매개변수에 부모타입 참조변수를 작성하면 모든 자식 객체를 전달받을 수 있다.
	public void printCar(Car temp) {
		// 부모타입 참조변수 = 자식타입 객체
		// 다형성의 업캐스팅 모양과 똑같다
		// => temp 에는 Tesla, Spark, Car 의 주소값이 넘어와도 된다
		//				--> 업캐스팅(UpCating)
	...
	public Car createCar(int num) {		// 반환형 -> 부모타입
	...
	public void ex2() {
		...
		Spark s = new Spark("경차엔진", "가솔린", 4, 0.5);
		
		Car c = new Car("경유엔진", "디젤", 12);
	...
	public void ex3() {
		...
		// 소괄호를 사용해서 형변환을 먼저 하고 getBatteryCapacity()를 꺼내온다(순서)
		
		Tesla t1 = (Tesla)c1;
		// Tesla 타입으로 강제형변환한 변수(c1)를 Tesla 타입의 변수(t1)에 대입
	...
	public void ex4() {
		...
		//	  Spark 참조변수로 Tesla 를 참조하려고 하면 문제 발생
	...
	public void ex5() {
		...
		// 프로그램 "실행 전"
		// - 컴파일러는 getEngine() 메서드가 Car 에 있는걸로 인식해서
		// c1.getEngine() 호출코드와
		...
		// 참조변수 c2 가 Car 타입이므로 toString() 메서드도
		// Car 의 toString() 호출 - 정적바인딩
		
		// 하지만 프로그램 실행시 자식(Spark)의 toString이 호출되는것을 확인 
