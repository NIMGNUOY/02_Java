...
public class JavaAPIService {
	...
	public void method1() {
		...
		// 6000
		// print 구문 내에서 Wrapper Class 로 만든 객체를 참조할 때
		// .toString() 메서드 호출하는 것이 아닌 Auto Unboxing 진행해서
		// 기본 자료형으로 변경한다.
	...
	public void method2 () {
		...
		// -> HTML 관련된 모든 값은 String
		// 단, char(Character)는 별도로 존재하지 않음.
		
		byte b = Byte.parseByte("1");	// ( HTML 에서 넘어온 값 )
	...
	public void method5() {
		...
		// --> new 연산자로 만든 객체는 새로운 객체를 만듬!!
		// 즉, 동일한 리터럴로 만들었을 때와 달리 다른 객체가 생성
		...
		// 기존 temp2 와 해시코드 다름
		//	--> 다른 객체(새로운 객체를 만들고 기존의 객체는 Garbage Collector 에)
	 ...
	 * 	  -> 마음대로 문자열 수정, 삭제 가능
	 * 	  -> 수정, 삭제를 해도 추가적인 객체 생성이 없어서 효율 좋음
	 *    
	 *  
	 *  - StringBuilder : Thread Safe 미제공 (비동기화)
	 *  : 속도면에서는 StringBuffer 보다 성능이 좋음.
	 *    -> 멀티쓰레드 환경에서는 StringBuilder 사용 시 쓰레드 충돌 가능성 있음
	 *    							--> 글자가 깨지거나 오류가 발생 
	 *    -> 즉, 단일 쓰레드 환경에서 유리( 추천 )
	 *  
	 *  - StringBuffer : Thread Safe 제공 (동기화)
	 *  : 속도면에서는 StringBuilder 보다 성능 떨어짐.
	 *    -> 멀티쓰레드 환경에서 안전하게 동작할 수 있음.
	 *    -> 즉, 멀티쓰레드 환경에서 유리함.
	 *  
	 *  
	 *    (동기 : 이전 명령을 다 수행한 후 다음 명령 수행
	 *    cf. 비동기 : 이전 명령을 수행하면서 여유가 있을 때 다음 명령 같이 수행)
	 */
	public void method6() {
		...
		sb.insert(0/*0번 index*/, "2월 21일 ");	// 0번 인덱스에 삽입 == 제일 앞
		System.out.println(System.identityHashCode(sb));	// 1865127310
		// 같은 주소 참조 --> 즉, 새로운 객체를 만드는 것이 아닌,
		//						  객체를 덮어쓰는 것이 아닌 기존 객체의
		//						  값을 변경하여 저장함
