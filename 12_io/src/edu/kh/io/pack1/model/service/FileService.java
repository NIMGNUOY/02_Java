...
public class FileService {
	 ...
	 * - 경로 표기 방법
	 * 
	 * 1) 절대 경로 : 하나의 (절대적인) 기준으로 부터 목표까지의
	 * 				경로를 모두 표기하는 방법
	 * 
	 * - 기준
	 * [Windows] : 	C:/	  ,    D:/
	...
	public void method1() {
		...
		if( !directory.exists() ) { 	// 폴더가 존재하지 않으면
			...
			// 폴더 모두 생성
			directory.mkdirs();		// makes directories
	...
	public void method3() {
		...
		// 지정된 디렉토리에 있는 모든 파일, 디렉토리를 File[] 
		// 형태로 얻어오기
		File[] files = directory.listFiles();
		
		// 향상된 for 문 -> for( 요소 저장변수 : 배열 || 컬렉션 ) {}
		for(File f : files) {
			...
			// String SimpleDateFormat.format( long ) : 매개변수 long 값을 지정된 패턴
			//										형식으로 변화하여 문자열 형태로 반환
