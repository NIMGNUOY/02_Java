...
public class ByteService {
	...
	public void fileByteOutput() {
		...
		try {
			...
			// StringBuilder.append() or StringBuilder.insert()
			...
			// -> 2byte 범주의 문자(영어, 숫자, 기본 특수문자 제외)는 1byte 기반 스트림 
			//   통과 시 데이터 손실 발생하여 글자가 깨지는 문제 발생
		...
		} finally { // try 예외 발생여부 관계 없이 무조건 수행
	...
	public void bufferedFileByteOutput() {
		 ...
		 * -> 동영상 재생 중에 미리 앞에 재생될 부분을 
		 *   다운 받아놔서 영상을 끊김없이 볼 수 있게 함
		 * -> 다운받아서 저장해놓는 공간 == 버퍼
		 * 
		 * 
		 */
		 ...
		 * 
		 * - FileOutputStream 은 1byte 씩 데이터를 입출력
		 * -> 버퍼에 입출력할 내용을 모아서 한 번에 입출력!
		 * 
		 * --> Stream 을 이용하는 횟수가 적어짐
		 * 	-> 성능, 시간 효율 상승
		 * 
		 */
	...
	public void fileByteInput() {
		...
		try {
			...
			// InputStream 은 flush() 없음	cf) OutputStream
	...
	public void fileCopy() {
		...
		try {
			...
			// target 이 가리키는 파일을 입력 받을 수 있도록 입력용 스트림을 생성
			...
			// 출력할 파일의 경로 + _copy 가 붙은 파일 이름 만들기
			...
			// - String 뒤에서부터 검색해서 "문자열"과 일치하는 부분의 인덱스 반환
			// 없으면 -1 반환
			
			int insertPoint = target.lastIndexOf(".");
			sb.insert(insertPoint, "_copy");	// 매개변수로 들어간 index에
												// 문자열 삽입
			...
			bos = new BufferedOutputStream( fos );	// 보조스트림 연결
			
			// 읽어왔던 내용 bytes 를 bos 를 이용해서 출력
			bos.write(bytes);
			...
			try {
				// 사용한 스트림 메모리 모두 반환
				if( br != null ) br.close();
				if( bis != null) bis.close();	// 메인 스트림도 같이 닫힘
				if( bos != null ) bos.close();	// 메인 스트림도 같이 닫힘
