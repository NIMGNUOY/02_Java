...
public class TodoListServiceImpl implements TodoListService {
	...
	public Map<String, Object> todoListFullView() {
		...
		// 1. 할 일 목록 DAO 에서 얻어오기
		List<Todo> todoList = dao.todoListFullView();
		
		// 2. 할 일 목록에서 완료(complete 필드값이 true)인 요소가 몇개인지 카운트
		int completeCount = 0;
	...
	public String todoDetailView(int index) {
		...
		// 1. DAO 에 있는 todoList 에서 index 번째 요소(Todo) 반환 받기
		//   없으면 null 반환
		Todo todo = dao.todoDetailView( index );
		
		// 2. 반환받은 todo 가 null 인 경우 null 반환
	...
	public String todoDelete(int index) throws Exception {
		...
		else return todo.getTitle();	// 제목 반환
	}
